...
private[episode] class DeleteRecentlyFinishedEpisodesAction(episodesAvailable: => Boolean)(implicit val bindingModule: BindingModule)
  ...
  override protected def processData(context: Context, data: Unit): Unit = {
    inject[DownloadService].deleteRecentlyFinished()
  }
}
